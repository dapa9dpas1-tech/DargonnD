...
if ($action === 'admin_save_product' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image_file'])) {
        ...
        if ($err === UPLOAD_ERR_OK) {
            ...
            flash_set('Upload error code: ' . $err, 'error');
            header('Location: ?page=admin_products'); exit;
        }
    }
    
    // Fallback if adding new and no image uploaded
    if ($id === 0 && $imagePath === '') {
        // We could require an image or just allow empty.
        // Let's allow empty, but it might break the UI if not handled.
    }
